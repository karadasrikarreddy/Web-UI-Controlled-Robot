...
public:
    Costmap2DFrom3D() : Node("costmap_2d_from_3d") {
        ...
        // 1. Subscribe to 3D Map (Keep this Transient Local to hear the Map Manager)
        ...
        // 2. Publish 2D Costmap (VOLATILE MODE)
        // We use simple '10' which is Standard/Volatile. 
        // This makes it compatible with Web UI and standard Subscribers.
        pub_2d_costmap_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("/costmap_2d", 10);

        RCLCPP_INFO(this->get_logger(), "✅ Costmap Node Ready (Volatile Mode)");
    }
...
private:
    void map_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        RCLCPP_INFO(this->get_logger(), "✅ Map Published: %dx%d", width, height);
    }
